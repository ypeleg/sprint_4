            ...
            <div className="table-scroll-container">
                <div className="group-table">
                    ...
                    <Droppable droppableId="group-table">
                        {(provided) => (<div className="table-body" ref={provided.innerRef}{...provided.droppableProps}>
                                {flatTasks.map((task, index) => (<Draggable key={task.id} draggableId={task.id} index={index}>
                                        {(provided) => (<div ref={provided.innerRef}{...provided.draggableProps} {...provided.dragHandleProps} className="table-row board-table">
                                                <div className="col-1">
                                                    <div className="status-indicator">
                                                        {task.status === "done" && (<svg width="16px" height="16px" fill="none" viewBox="0 0 16 16" role="presentation" className="check-icon">
                            ...
                            {task.groupTitle}
                                                ...
                                                <div className="col-3-labels">
                                                    <div className="labels labels-table">
                                                        {task.labels.map((label) => (<div key={label.id} className="task-label tooltip" style={{backgroundColor: label.color || ""}} data-tip={label.title}></div>))}
                                                    </div>
                                                </div>
                                                <div className="col-4-members">
                                                    <div className="task-users">
                                                        {task.members?.length > 0 && (<div className="task-user-icons task-user-icon">
                                                ...
                                                <div className="col-5-due-date">
                                                    {task.dueDate && (<div className="task-date tooltip" data-tip="Due date">
