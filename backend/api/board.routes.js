...
export const boardService = {
    ...
    getById: async function (boardId, filterBy) {
        try {
            ...
            // const board = await collection.aggregate().toArray()
            return board[0]
        } catch (err) {
    ...
    async save(board, loggedinUser) {
        try {
            ...
            if (board._id) {
                ...
                await collection.updateOne({_id: new ObjectId(boardId)}, {$set: boardToUpdate})
...
export async function onAddboard(req, res) {
    ...
    try {
        ...
        socketService.broadcast({
            type: 'board-added', data: addedBoard, userId
        })
        res.json(addedBoard)
    } catch (err) {
...
export async function onUpdateboard(req, res) {
    try {
        ...
        const userId = (SECURED && req.loggedinUser) ? req.loggedinUser._id : 'anonymous'
        socketService.broadcast({
            type: 'board-updated', data: updatedBoard, userId
        })
...
export async function onRemoveboard(req, res) {
    try {
        ...
        if (deletedCount > 0) {
            const userId = (SECURED && req.loggedinUser) ? req.loggedinUser._id : 'anonymous'
            socketService.broadcast({
                type: 'board-removed', data: boardId, userId
            })
        }
    ...
    return [{$match: {_id: new ObjectId(boardId)}},
            ...
            ...(title ? [{
                $set: {
                    "groups": {
                        $map: {
                            input: "$groups", as: "group", in: {
                                $mergeObjects: ["$$group", {
                                    tasks: {
                                        $filter: {
                                            input: "$$group.tasks", as: "task", cond: {
                                                $regexMatch: {
                                                    input: "$$task.title", regex: new RegExp(title, 'i'),
                                                }
                                            }
                                        }
                                    }
                                ...
                                $mergeObjects: ["$$group", {
                                    tasks: {
                                        $filter: {
                                            input: "$$group.tasks", as: "task", cond: {
                                                $or: [{
                                                    $and: [{$eq: [{$size: "$$task.members"}, 0]}, {$in: ["1", members]}]
                                                }, {
                                                    $in: [{$arrayElemAt: ["$$task.members._id", 0]}, members]
                                                }]
                                            }
                                        }
                                    }
                                        ...
                                        $filter: {
                                            input: "$$group.tasks", as: "task", cond: {
                                                $or: [
                                                    ...
                                                    {
                                                        $and: [{$eq: [{$ifNull: ["$$task.dueDate", null]}, null]}, {$in: ["no", dueDate]}]
                                                    },

                                                    {
                                                        $and: [{$in: ["week", dueDate]}, {
                                                            $gt: [{$dateFromString: {dateString: "$$task.dueDate"}}, new Date()]
                                                        }, {
                                                            $lt: [{$dateFromString: {dateString: "$$task.dueDate"}}, new Date(new Date().setDate(new Date().getDate() + 7))]
                                                        }
                                                    ...
                                                    {
                                                        $and: [{$in: ["over", dueDate]}, {
                                                            $lt: [{$dateFromString: {dateString: "$$task.dueDate"}}, new Date()]
                                                        }]
                                                    }]
                                            }
                                        }
                                ...
                                $mergeObjects: ["$$group", {
                                    tasks: {
                                        $filter: {
                                            input: "$$group.tasks", as: "task", cond: {
                                                $cond: {
                                                    if: {$eq: [status, "done"]}, then: {$eq: ["$$task.status", "done"]}, else: {$ne: ["$$task.status", "done"]}
                                                }
                                            }
                                        }
                                    }
...
async function routOnAddboard(req, res, next) {
    req.route.func = 'addboard';
    next()
}

async function routOnGetboards(req, res, next) {
    req.route.func = 'getboards';
    next()
}

async function routOnRemoveboard(req, res, next) {
    req.route.func = 'removeboard';
    next()
}

async function routOnUpdateboard(req, res, next) {
    req.route.func = 'updateboard';
    next()
}

async function routOnGetboardById(req, res, next) {
    req.route.func = 'getboardById';
    next()
}
